...
const logoBannerPath = path.join(prototypeDir, 'public/logo-banner-dark.png');
const logoBannerBase64 = `data:image/png;base64,${fs.readFileSync(logoBannerPath).toString('base64')}`;

const htmlPath = path.join(__dirname, 'rankur-deck.html');
const pdfOutputPath = path.join(prototypeDir, 'RANKUR_COMPANY_PROFILE_DECK.pdf');
...
<head>
  ...
  <style>
    @import url('https://fonts.googleapis.com/css2?family=Cinzel:wght@700;800&family=Inter:wght@400;500;600;700;800&family=Playfair+Display:ital,wght@0,600;0,700;0,800;1,400;1,600&display=swap');
  ...
  <section class="slide slide--dark">
    <div>
      <div style="margin-bottom: 25px;">
        <img src="${logoBannerBase64}" alt="RankurSite.com Logo" style="height: 110px; width: auto; object-fit: contain;" />
      </div>
      <div class="eyebrow" style="color: #C9A84C; margin-bottom: 30px;">
    ...
    <div>
      ...
      <div class="cap-cols">
        ...
        <div class="cap-col">
          ...
          <p class="cap-desc">
            We don't hide behind empty claims without proof. Prospective clients can inspect live production platforms (WizIOT, Atlanta Systems, Medventa, Probiota Innovations) and click through the verified <em>"Designed and built by Rankur"</em> footer links.
          </p>
        </div>
      </div>
    </div>
...
const command = \`Start-Process -FilePath "\${edgePath}" -ArgumentList '--headless', '--disable-gpu', '--print-to-pdf="\${pdfOutputPath}"', '--no-pdf-header-footer', '"file:///\${htmlPath.replace(/\\\\/g, '/')}"' -Wait\`;

execSync(\`powershell -Command "\${command}"\`);
console.log('PDF rendered successfully to:', pdfOutputPath);
`;

fs.writeFileSync(path.join(__dirname, 'build-deck-pdf.js'), CodeContent);
